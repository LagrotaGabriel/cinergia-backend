...
public class ClienteEntity {
    ...
    @Column(table = "TB_SBS_CLIENTE", name = "COD_CLIENTE_CLI", nullable = false, updatable = false, length = 36)
    private UUID uuid;
    ...
    @JoinColumn(table = "TB_SBS_CLIENTE", name = "COD_EMPRESA_CLI", nullable = false, updatable = false)
    private EmpresaEntity empresa;

    @Column(table = "TB_SBS_CLIENTE", name = "COD_ASAASID_CLI", nullable = false, unique = true)
    private String asaasId;

    @Comment("Data em que o cadastro do cliente foi realizado")
    @Column(table = "TB_SBS_CLIENTE", name = "DT_DATACADASTRO_CLI", nullable = false, updatable = false, length = 10)
    private String dataCadastro;

    @Comment("Hora em que o cadastro do cliente foi realizado")
    @Column(table = "TB_SBS_CLIENTE", name = "HR_HORACADASTRO_CLI", nullable = false, updatable = false, length = 18)
    private String horaCadastro;

    @Comment("Nome do cliente")
    @Column(table = "TB_SBS_CLIENTE", name = "STR_NOME_CLI", nullable = false, length = 70)
    private String nome;

    @Comment("E-mail do cliente")
    @Column(table = "TB_SBS_CLIENTE", name = "EML_EMAIL_CLI", length = 70)
    private String email;

    @Comment("CPF ou CNPJ do cliente")
    @Column(table = "TB_SBS_CLIENTE", name = "CDP_CPFCNPJ_CLI", nullable = false, unique = true, length = 18)
    private String cpfCnpj;

    @Comment("Observações sobre o cliente")
    @Column(table = "TB_SBS_CLIENTE", name = "STR_OBSERVACOES_CLI", length = 300)
    private String observacoes;

    @Enumerated(EnumType.STRING)
    @Comment("Status atual do cliente: 0 - Comum, 1 - Em débito, 2 - Vip, 3 - Atenção")
    @Column(table = "TB_SBS_CLIENTE", name = "ENM_STATUS_CLI", nullable = false)
    private StatusClienteEnum statusCliente;

    @Comment("Caso a pessoa seja FÍSICA, a data de nascimento poderá ser informada")
    @Column(table = "TB_SBS_CLIENTE", name = "DT_DATANASCIMENTO_CLI", length = 10)
    private String dataNascimento;

    @Enumerated(EnumType.STRING)
    @Comment("Tipo de pessoa do cliente: 0 - Física, 1 - Jurídica")
    @Column(table = "TB_SBS_CLIENTE", name = "ENM_TIPOPESSOA_CLI", nullable = false)
    private TipoPessoaEnum tipoPessoa;

    @Comment("Objeto contendo informações de acesso ao sistema do cliente")
    @Column(table = "TB_SBS_CLIENTE", name = "COD_ACESSO_CLI")
    ...
    @Column(table = "TB_SBS_CLIENTE", name = "COD_EXCLUSAO_CLI")
    ...
    @Column(table = "TB_SBS_CLIENTE", name = "COD_ENDERECO_CLI")
    ...
    @Column(table = "TB_SBS_CLIENTE", name = "COD_FOTOPERFIL_CLI")
    ...
    @Column(table = "TB_SBS_CLIENTE", name = "LST_PLANOS_CLI")
    ...
    @Column(table = "TB_SBS_CLIENTE", name = "LST_TELEFONES_CLI")
